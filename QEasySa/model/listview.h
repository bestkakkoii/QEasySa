...
public:
	...
	void __vectorcall append(const QString& str);
	void __vectorcall append(const QStringList& str);

	int __vectorcall size() const { QReadLocker locker(&m_stringlistLocker); return m_list.size(); }

	QString __vectorcall takeFirst();

	void __vectorcall remove(const QString& str);

	void __vectorcall clear();

	void __vectorcall setStringList(const QStringList& list);

	QStringList __vectorcall getStringList() const { QReadLocker locker(&m_stringlistLocker); return m_list.toList(); }


	void __vectorcall swapRowUp(int source);
	void __vectorcall swapRowDown(int source);
protected:
